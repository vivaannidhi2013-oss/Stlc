...
=============================================================
 Key upgrades in this version:
   ✅ Domain-aware prompt — understands Kafka, IBAN, BIC, routing logic
   ...
   ✅ Realistic domain-specific test data (real IBAN/BIC formats)
...
def clean_json_string(text: str) -> str:
    """
    Fixes the most common JSON syntax errors that LLMs produce:

    1. Trailing commas before ] or }  →  the #1 cause of JSONDecodeError
       e.g.  ["a", "b",]  becomes  ["a", "b"]

    2. Single-quoted strings  →  Claude sometimes uses 'value' instead of "value"
       Only fixes standalone single-quoted values, not apostrophes inside strings.

    3. Strips markdown code fences  →  ```json ... ```

    4. Extracts the JSON object if Claude added any preamble/postamble text.
    """
    ...
    # Step 2: Extract JSON object — find outermost { ... }
    start = text.find("{")
    end   = text.rfind("}") + 1
    ...
    # Handles:  ,}  ,]  , }  , ]  including newlines between
    text = re.sub(r",\s*(\}|\])", r"\1", text)

    # Step 4: Fix single-quoted string values (basic cases only)
    # Replaces: 'value'  with  "value"  when clearly a JSON string
    # Uses a careful pattern to avoid breaking apostrophes inside text
...
def parse_response(raw: str) -> dict:
    """
    Parses Claude's JSON response with multi-stage fallback:
      Stage 1 → Try parsing as-is (fastest path)
      Stage 2 → Strip code fences and retry
      Stage 3 → Full clean_json_string() repair and retry
      Stage 4 → Save raw to file for manual inspection and raise
    """
    ...
    # Stage 2: Strip code fences and try again
    if text.startswith("```"):
        lines = text.split("\n")
    ...
    # Stage 3: Full repair pass
    print("⚠️  Initial parse failed — running JSON repair...")
    ...
    try:
        ...
        # Stage 4: Save raw for debugging and raise a clear error
        debug_file = "claude_raw_response_debug.txt"
        with open(debug_file, "w", encoding="utf-8") as f:
            f.write(raw)
        print(f"\n❌ JSON parse failed even after repair.")
        print(f"   Error location: {e}")
        print(f"   Full raw response saved to: {debug_file}")
        print(f"   Open that file, find line {e.lineno} col {e.colno}, and look for:")
        print(f"   → Trailing comma before }} or ]")
        print(f"   → Unescaped double-quote inside a string value")
        print(f"   → Truncated response (response cut off mid-JSON)")
        raise
